...
import torch_compile_normal_dtype_promotion_guard.core as core
from torch_compile_normal_dtype_promotion_guard.cli import main
...
def test_version_flag(capsys):
    ...
    assert code == 0
    assert "torch-compile-normal-dtype-promotion-guard" in out
...
def _fake_case(diverges, guard_ok, loc_dtype="torch.float16", scale_dtype="torch.float32"):
    eager_dtype = loc_dtype
    compiled_dtype = scale_dtype if diverges else loc_dtype
    guarded_dtype = eager_dtype if guard_ok else compiled_dtype
    return {
        "loc_dtype": loc_dtype,
        "scale_dtype": scale_dtype,
        "eager_dtype": eager_dtype,
        "compiled_dtype": compiled_dtype,
        "dtype_diverges": diverges,
        "guarded_dtype": guarded_dtype,
        "guarded_matches_eager": guard_ok,
    }


def _fake_report(diverges, guard_ok):
    return {
        "torch_version": "0.0.0-fake",
        "issue_url": "https://github.com/pytorch/pytorch/issues/194547",
        "cases": [_fake_case(diverges, guard_ok)],
        "any_native_divergence": diverges,
...
def test_no_bugs_shows_info_message_not_fail(capsys, monkeypatch):
    ...
    code = main(["--no-color"])
    out = capsys.readouterr().out
    assert "[i] no Normal.sample() dtype-promotion divergence reproduced" in out
    assert code == 0
...
def test_guard_failed_label_shown_when_guard_ineffective(capsys, monkeypatch):
    ...
    assert "[X] guard did NOT restore eager's dtype contract on at least one case" in out
    assert code == 1
...
def test_bug_reproduced_shows_fail_headline_and_ok_guard(capsys, monkeypatch):
    ...
    code = main(["--no-color"])
    out = capsys.readouterr().out
    assert "[X] torch.compile Normal.sample() dtype-promotion divergence reproduced on this host (pytorch#194547)" in out
    assert "[OK] safe_compiled_normal_sample() restores eager's dtype-preservation contract on every case" in out
    assert code == 0
